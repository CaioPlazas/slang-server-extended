...
void SlangDoc::addMacroOwner(std::shared_ptr<SlangDoc> owner) {
    for (auto& existing : m_macroOwners) {
        if (existing == owner)
            return;
    }
    m_macroOwners.push_back(std::move(owner));

    // The next getSyntaxTree() call needs to reparse with the new macro predefined -- drop the
    // cached tree so hasValidBuffers()'s staleness check isn't what decides whether that happens.
    m_tree.reset();
}

std::vector<const syntax::DefineDirectiveSyntax*> SlangDoc::collectInheritedMacros() const {
    std::vector<const syntax::DefineDirectiveSyntax*> macros;
    for (auto& owner : m_macroOwners) {
        auto ownerTree = owner->getSyntaxTree();
        if (!ownerTree)
            continue;
        auto defined = ownerTree->getDefinedMacros();
        macros.insert(macros.end(), defined.begin(), defined.end());
    }

    // Append the driver-level macro pool (singleUnitMacros experimental feature) after any
    // macro-owner macros above -- slang's preprocessor resolves duplicate names first-wins, so
    // listing owner-specific macros first means they still win over the broader pool for the
    // same name; the pool only fills in names no owner already defines. Macros this file itself
    // contributed are excluded, or an include-guarded file would predefine its own guard and
    // parse to an empty body.
    m_driver.appendMacroPool(m_uri, macros);
...
std::shared_ptr<syntax::SyntaxTree> SlangDoc::getSyntaxTree() {
    ...
    if (!m_tree) {
        ...
        auto inheritedMacros = collectInheritedMacros();
        m_tree = syntax::SyntaxTree::fromBuffer(m_buffer, m_sourceManager, m_options,
                                                inheritedMacros);
        m_macroGen = m_driver.getMacroPoolGen();
    }
    else if (!hasValidBuffers(m_sourceManager, m_tree) ||
            m_macroGen != m_driver.getMacroPoolGen()) {
        // Tree has invalid buffers, or the driver's singleUnitMacros pool changed since we last
        // parsed against it -- need to reparse either way.
        m_buffer = m_sourceManager.readSource(m_uri.getPath(), nullptr).value();
        auto inheritedMacros = collectInheritedMacros();
        m_tree = syntax::SyntaxTree::fromBuffer(m_buffer, m_sourceManager, m_options,
                                                inheritedMacros);
        m_macroGen = m_driver.getMacroPoolGen();
        // The analysis was built from the old tree and the now-superseded buffer id. Leaving it
        // in place lets diagnostics get silently dropped: issueDiagnosticsTo filters by
        // m_buffer.id, which no longer matches the buffer the stale analysis reported against.
        m_analysis.reset();
    }
    return m_tree;
}

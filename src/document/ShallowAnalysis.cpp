...
namespace {
/// A `virtual apb_if vif;` handle's type is VirtualInterfaceType, which is not itself a Scope
/// (the real interface members live on the InstanceSymbol it points at) -- so plain
/// `type.isScope()` never fires for it. Route member lookups on the handle (e.g. `vif.psel`) to
/// the modport's scope when the handle is modport-restricted, otherwise the full interface body.
const ast::Scope* scopeForType(const ast::Type& type) {
    if (type.isVirtualInterface()) {
        auto& vif = type.as<ast::VirtualInterfaceType>();
        if (vif.modport) {
            return vif.modport;
        }
        return &vif.iface.body.as<ast::Scope>();
    }
    if (type.isScope()) {
...
const ast::Scope* ShallowAnalysis::getScopeFromSym(const ast::Symbol* symbol) const {
    ...
    if (symbol->isType()) {
        // unwrapErrorType returns the canonical type, and recovers the intended type from a
        // recoverable error -- so this keeps working on half-typed code. scopeForType then adds
        // the virtual-interface routing that plain isScope() misses.
        return scopeForType(unwrapErrorType(symbol->as<ast::Type>()));
    }
    else if (ast::ValueSymbol::isKind(symbol->kind)) {
        return scopeForType(unwrapErrorType(symbol->as<ast::ValueSymbol>().getType()));
    }

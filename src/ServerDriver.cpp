...
void ServerDriver::parseAndLoadSources(const std::vector<std::string>& buildfiles) {
    ScopedTimer timer("Parsing build sources");
    ...
    for (auto& tree : driver.syntaxTrees) {
        ...
        m_buildOrder.push_back(uri);
        // These trees were parsed before any pool existed, so they're already guard-free and can
        // seed the harvest directly -- startup needs no second parse.
        harvestFile(uri, tree);
        auto doc = SlangDoc::fromTree(*this, std::move(tree));
        docs[uri] = doc;
    }

    // The pool is harvested from the files the driver actually parsed. Those can come from a
    // build/flist OR from positional sources in `flags`, so the check that matters is whether
    // anything got parsed at all -- not whether a `build` happens to be configured.
    if (m_config.experimental.value().singleUnitMacros.value() && m_buildOrder.empty()) {
        WARN("experimental.singleUnitMacros is enabled but no source files were parsed, so the "
             "macro pool is empty and the option has no effect -- configure a build/flist or list "
             "sources in `flags`");
    }
...
namespace {
/// Raw source text of a macro directive. Used only for change detection -- far cheaper than
/// SyntaxNode::toString(), which renders the whole body (trivia included) into a fresh string
/// for every macro on every rebuild.
std::string_view macroSourceText(const SourceManager& sm,
                                 const syntax::DefineDirectiveSyntax* macro) {
    auto range = macro->sourceRange();
...
void ServerDriver::harvestFile(const URI& uri, std::shared_ptr<syntax::SyntaxTree> tree) {
    if (!m_config.experimental.value().singleUnitMacros.value())
        return;

    if (!tree) {
        // Re-read from disk. replaceBuffer() has already repointed this path's cache entry at the
        // edited text, so this picks up the new contents under a fresh BufferID.
        auto buffer = sm.readSource(uri.getPath(), nullptr);
        if (!buffer)
            return;

        // Empty inherited-macro list is the whole point: this parse must see the file the way a
        // standalone compile would, so its `\`ifndef` guards evaluate normally. Harvesting from a
        // pool-parsed tree instead is what made guarded headers (uvm_macros.svh) self-suppress and
        // drop their macros. maxIncludeDepth stays at the default so `\`include`d macro headers are
        // still followed, and options keeps the flist's `+define+`s so `\`ifdef`-conditional
        // `\`define`s resolve the way the real build sees them.
        tree = syntax::SyntaxTree::fromBuffer(*buffer, sm, options, {});
    }
    if (!tree)
        return;

    HarvestedFile harvested;
    harvested.tree = tree;
    auto bufferIds = tree->getSourceBufferIds();
    // Pin the source TEXT as well as the nodes: the tree owns its DefineDirectiveSyntax nodes via
    // its BumpAllocator, but their tokens' string_views point into SourceManager-owned memory that
    // the tree does not reference-count, and replaceBuffer() frees it on the next edit. Must be
    // captured now, while the tree is fresh -- retainBuffers() skips already-released entries.
    harvested.retainedBuffers = sm.retainBuffers(bufferIds);

    for (auto bufId : bufferIds) {
        auto path = sm.getFullPath(bufId);
        if (!path.empty())
            harvested.sourceFiles.push_back(URI::fromFile(path));
    }

    for (auto macro : tree->getDefinedMacros()) {
        if (macro->name.valueText().empty())
            continue;

        // Anything defined outside this tree's own buffers is a flist `+define+`, which `options`
        // already predefines for every parse -- pooling it again would be redundant. Unlike the
        // old filter this is now stable, because a guard-free tree never inherits anything.
        auto macroBuffer = macro->sourceRange().start().buffer();
        if (std::find(bufferIds.begin(), bufferIds.end(), macroBuffer) == bufferIds.end())
            continue;

        harvested.macros.push_back(macro);
    }

    // Retire the previous harvest instead of letting the assignment destroy it -- see
    // m_retiredHarvests. Documents parsed against the old pool still point into its buffers.
    if (auto it = m_harvested.find(uri); it != m_harvested.end())
        m_retiredHarvests.push_back(std::move(it->second));

    m_harvested[uri] = std::move(harvested);
}

void ServerDriver::rebuildMacroPool() {
    if (!m_config.experimental.value().singleUnitMacros.value())
        return;

    ScopedTimer timer("Rebuilding macro pool");

    // Last-wins in flist order: iterating m_buildOrder in order and overwriting on each occurrence
    // means a later file's `\`define` of the same name naturally replaces an earlier one's.
    flat_hash_map<std::string_view, std::pair<const URI*, const syntax::DefineDirectiveSyntax*>>
        byName;
    for (auto& uri : m_buildOrder) {
        auto it = m_harvested.find(uri);
        if (it == m_harvested.end())
            continue;
        for (auto macro : it->second.macros)
            byName[macro->name.valueText()] = {&it->first, macro};
    }

    std::vector<std::pair<const URI*, const syntax::DefineDirectiveSyntax*>> newPool;
    newPool.reserve(byName.size());

    // Order-independent content hash: combine each (name, raw source text) pair into its own
    // sub-hash, then aggregate with `+=` so the total doesn't depend on byName's (unordered)
    // iteration order. Two rebuilds with the same macro set must hash equal.
    size_t hash = 0;
    for (auto& [name, entry] : byName) {
        newPool.push_back(entry);

        size_t h = std::hash<std::string_view>{}(name);
        h ^= std::hash<std::string_view>{}(macroSourceText(sm, entry.second)) + 0x9e3779b9 +
             (h << 6) + (h >> 2);
        hash += h;
    }

    // The pool must ALWAYS be re-pointed at the current harvest, even when the content is
    // identical. A re-harvest replaces its file's entry in m_harvested, destroying the previous
    // SyntaxTree -- so every DefineDirectiveSyntax the old pool held for that file is dangling the
    // moment harvestFile() returns. Returning early here (as this did) left those freed pointers
    // in m_macroPool for the next parse to feed to the preprocessor: a segfault on roughly every
    // other save, since an unchanged macro set is exactly the common case.
    m_macroPool = std::move(newPool);

    if (hash == m_macroPoolHash) {
        // Content unchanged (e.g. a save that didn't touch any `\`define`) -- don't bump the
        // generation counter, or every doc would needlessly reparse. The pool above is already
        // refreshed, so this only suppresses the downstream reparse.
        return;
    }

    m_macroPoolHash = hash;
    ++m_macroPoolGen;
}

void ServerDriver::appendMacroPool(const URI& ownUri,
                                   std::vector<const syntax::DefineDirectiveSyntax*>& out) const {
    out.reserve(out.size() + m_macroPool.size());
    for (auto& [uri, macro] : m_macroPool) {
        // A file's own macros execute during its own parse. Predefining them would make an
        // include-guarded file's `\`ifndef` false and collapse its entire body into an inactive
        // region -- taking its classes out of the compilation with it.
        if (uri && *uri == ownUri)
            continue;
        out.push_back(macro);
    }
...
void ServerDriver::updateDoc(SlangDoc& doc, FileUpdateType type) {
    if (type == FileUpdateType::SAVE) {
        // Only the saved file can have changed, so re-harvest just that one rather than walking
        // the whole build. Skipped entirely unless it's a pool contributor that actually contains
        // a `\`define` -- otherwise the harvest (and the full-build reparse a pool change causes)
        // would run on every save of every file.
        auto& uri = doc.getURI();
        if (m_config.experimental.value().singleUnitMacros.value()) {
            // Re-harvest every build file that read this path -- the file itself if it's in the
            // build, plus any build file that `\`include`s it (editing uvm_macros.svh has to
            // refresh the packages that pull it in, not just itself).
            //
            // Deliberately not gated on the saved text containing a `\`define`: a build file can
            // change the pool purely by adding or removing an `\`include` of a macro header. The
            // cost is one re-parse of the affected file(s), not the whole build, and the pool's
            // content hash still decides whether anything actually reparses downstream.
            std::vector<URI> stale;
            for (auto& [harvestedUri, harvested] : m_harvested) {
                if (std::find(harvested.sourceFiles.begin(), harvested.sourceFiles.end(), uri) !=
                    harvested.sourceFiles.end()) {
                    stale.push_back(harvestedUri);
                }
            }
            for (auto& staleUri : stale)
                harvestFile(staleUri);
...
namespace {
/// Classifies a file's standalone parse to decide whether it should be analyzed on its own or
/// bound to an `\`include`-owner's context instead. See resolveDocument, openDocument, and
/// adoptOrphanFragments for the three places this drives.
enum class StandaloneParse {
    /// Genuinely empty/garbage on its own -- a real `\`include`-only fragment. Always bound to its
    /// owner(s) when one is known.
    Fragment,
    /// Declares only classes (no module/interface/program/package). This is common UVM shape: a
    /// `.svh` with `class Foo extends ...; ... endclass` meant to be `\`include`d inside a
    /// `package`. Whether this is treated as fragment-eligible depends on whether the include site
    /// is nested inside a design unit in some owner's tree -- see includedInsideDesignUnit -- and
    /// is gated behind config.experimental.uvmVerificationLinting.
    NeedsOwnerContext,
    /// A genuinely valid, independent standalone compilation unit. Never bound to an owner.
    Standalone,
};

/// See StandaloneParse. Deliberately parse-only and cheap: both getMetadata() and diagnostics()
/// are populated as a side effect of parsing, so this needs no tree walk and no compilation.
/// doc->getSymbols() would go through getAnalysis(), which resolves dependent documents and builds
/// a full ShallowAnalysis -- this runs on the doc-open/getDocument hot path for every file in the
/// build, so it must stay parse-only.
StandaloneParse classifyStandaloneParse(const std::shared_ptr<SlangDoc>& doc) {
    if (!doc)
        return StandaloneParse::Fragment;
    auto tree = doc->getSyntaxTree();
    if (!tree)
        return StandaloneParse::Fragment;

    // (a) A declared module/interface/program/package makes this a real compilation unit -- even
    // if its body has errors, it's the file's own tree, not a fragment to splice elsewhere.
    for (auto& [decl, _] : tree->getMetadata().nodeMeta) {
        if (!decl->header->name.valueText().empty())
            return StandaloneParse::Standalone;
    }

    // (b) Declares only classes. This is the UVM shape -- may need an owner's context, may not,
    // decided later by whether an owner's include site is nested inside a design unit.
    for (auto classDecl : tree->getMetadata().classDecls) {
        if (!classDecl->name.valueText().empty())
            return StandaloneParse::NeedsOwnerContext;
    }

    // (c) No design unit, but if it parses cleanly its top-level members are all valid $unit-scope
...
/// True if `ownerTree` contains a class declaration from `fragBufId` (i.e. the fragment's own
/// buffer) that's nested inside a package/module/interface/program/class/checker, rather than
/// sitting directly at compilation-unit ($unit) scope. Used to decide whether a class-only `.svh`
/// (StandaloneParse::NeedsOwnerContext) should be bound to that owner -- the UVM
/// `` `include``-into-a-package pattern -- vs. left standalone (e.g. a class `\`include`d directly
/// at $unit scope, which is a different, unrelated usage).
bool includedInsideDesignUnit(const slang::syntax::SyntaxTree& ownerTree,
                              slang::BufferID fragBufId) {
    for (auto classDecl : ownerTree.getMetadata().classDecls) {
        if (classDecl->sourceRange().start().buffer() != fragBufId)
            continue;

        for (const slang::syntax::SyntaxNode* node = classDecl->parent; node;
             node = node->parent) {
            switch (node->kind) {
...
bool ServerDriver::isNestedInDesignUnit(const URI& uri, const std::shared_ptr<SlangDoc>& owner) {
    auto ownerTree = owner->getSyntaxTree();
    if (!ownerTree)
        return false;
    auto bufId = findBufferForPath(sm, *ownerTree, uri.getPath());
    if (!bufId)
        return false;
    return includedInsideDesignUnit(*ownerTree, *bufId);
}
...
void ServerDriver::openDocument(const URI& uri, const std::string_view text) {
    ...
    if (docIter != docs.end() && docIter->second->textMatches(text)) {
        ...
        if (!doc->isIncludeFragment()) {
            auto classification = classifyStandaloneParse(doc);
            bool eligible = classification == StandaloneParse::Fragment;
            bool needsOwnerContext = classification == StandaloneParse::NeedsOwnerContext &&
                                     m_config.experimental.value().uvmVerificationLinting.value();

            if (eligible || needsOwnerContext) {
                auto includeOwners = findIncludeOwners(uri);
                if (needsOwnerContext) {
                    std::vector<std::shared_ptr<SlangDoc>> nestedOwners;
                    for (auto& owner : includeOwners) {
                        if (isNestedInDesignUnit(uri, owner))
                            nestedOwners.push_back(owner);
                    }
                    if (nestedOwners.empty()) {
                        // See resolveDocument's matching comment: no owner nests this include in
                        // a design unit, so it can't be symbol-spliced, but its macros are still
                        // safe to inherit.
                        for (auto& owner : includeOwners) {
                            doc->addMacroOwner(owner);
                        }
                        alreadyInBuild = false;
                    }
                    includeOwners = std::move(nestedOwners);
                }
...
std::shared_ptr<SlangDoc> ServerDriver::resolveDocument(const URI& uri,
    ...
    auto classification = classifyStandaloneParse(standalone);
    if (classification == StandaloneParse::Standalone)
        return text ? SlangDoc::fromText(*this, uri, *text) : standalone;

    if (classification == StandaloneParse::NeedsOwnerContext) {
        if (!m_config.experimental.value().uvmVerificationLinting.value())
            return text ? SlangDoc::fromText(*this, uri, *text) : standalone;
        ...
        if (nestedOwners.empty()) {
            // No owner nests this include inside a design unit, so symbol-splicing isn't safe
            // (see includedInsideDesignUnit) -- but a `` `define`` an owner picked up along its
            // own `` `include`` chain (e.g. a class-only .svh `` `include``d at $unit scope,
            // right after a sibling `` `include "uvm_macros.svh"``) is still genuinely visible
            // to this file in a real compile, and doesn't carry the same symbol-scoping risk.
            auto doc = text ? SlangDoc::fromText(*this, uri, *text) : standalone;
            for (auto& owner : includeOwners) {
                doc->addMacroOwner(owner);
            }
            return doc;
        ...
        return SlangDoc::fromIncludeOwner(*this, uri, text, std::move(nestedOwners));
    }
...
void ServerDriver::adoptOrphanFragments(const std::shared_ptr<SlangDoc>& newOwner) {
    ...
    std::vector<std::shared_ptr<SlangDoc>> toAdoptMacrosOnly;
    for (auto& [uri, doc] : docs) {
        ...
        // standalone with real top-level content. Only adopt genuinely broken/empty parses, or
        // (gated behind config.experimental.uvmVerificationLinting) class-only files whose include
        // site in `tree` (newOwner's tree, already in hand) is nested inside a design unit --
        ...
        if (!doc->isIncludeFragment()) {
            ...
            if (classification == StandaloneParse::NeedsOwnerContext) {
                if (!m_config.experimental.value().uvmVerificationLinting.value())
                    continue;
                if (!includedInsideDesignUnit(*tree, *bufId)) {
                    // Can't splice symbols into newOwner's tree (see resolveDocument's matching
                    // comment), but newOwner's macros are still safe to inherit.
                    toAdoptMacrosOnly.push_back(doc);
                    continue;
                }
            }
        }
    ...
    for (auto& doc : toAdoptMacrosOnly) {
        doc->addMacroOwner(newOwner);
...
std::vector<std::shared_ptr<SlangDoc>> ServerDriver::getDependentDocs(
    ...
    while (!treesToProcess.empty()) {
        ...
        auto processRef = [&](std::string_view name) {
        ...
        meta.visitReferencedSymbols(processRef);

        // Virtual-interface member types and `extends` base class names are only followed
        // for opted-in verification workspaces -- see Config::Experimental. Kept behind a
        // separate ParserMetadata entry point (visitExtraReferencedSymbols) rather than
        // merged into visitReferencedSymbols, since that one has other consumers (the
        // Indexer's reverse-reference index, and slang's own SourceLoader/Driver) whose
        // behavior must not change by default.
        if (m_config.experimental.value().uvmVerificationLinting.value())
            meta.visitExtraReferencedSymbols(processRef);
    }

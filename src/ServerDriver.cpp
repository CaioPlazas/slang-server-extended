...
void ServerDriver::rebuildMacroPool() {
    ...
    // Re-entrancy guard: rebuilding calls getSyntaxTree() on every doc in the build, which can
    // trigger a reparse; that reparse's collectInheritedMacros() reads m_macroPool/m_macroPoolGen
    // via the driver's getters, which must keep returning the PREVIOUS pool throughout this
    // function (we only mutate m_macroPool/m_macroPoolTrees/m_macroPoolGen in one final step
    // below), so no real recursion hazard exists -- this guard is a defensive backstop against
    // reentry.
    if (m_rebuildingMacroPool)
        return;
    m_rebuildingMacroPool = true;
    auto guard = ScopeGuard([this] { m_rebuildingMacroPool = false; });

    struct Contribution {
        std::shared_ptr<syntax::SyntaxTree> tree;
        const syntax::DefineDirectiveSyntax* macro;
    };
    // Last-wins in flist order: iterating m_buildOrder in order and overwriting on each
    // occurrence means a later file's `\`define` of the same name naturally replaces an earlier
    // one's.
    flat_hash_map<std::string_view, Contribution> byName;
    ...
    for (auto& uri : m_buildOrder) {
        ...
        if (it == docs.end())
            continue;
        auto& doc = it->second;
        // Include fragments share their owner's tree -- skip them so we don't process the same
        // tree's macros twice (once via the owner, once via the fragment).
        ...
        // getDefinedMacros() returns the tree's *entire* active macro table at end of parse,
        // including macros it merely inherited (predefined) via collectInheritedMacros() --
        // e.g. from this very pool on a previous parse -- not just ones it locally `\`define`s.
        // Harvesting that unfiltered would let a macro re-contribute itself back into the pool
        // forever once inherited once, even after its real defining file stops defining it (a
        // doc that only *uses* an inherited macro would look like it "defines" it too). Filter
        // to macros whose defining directive actually lives in one of this tree's own buffers
        // (its own file or something it `\`include`s) -- an inherited macro's directive syntax
        // node belongs to the ORIGINATING tree's buffer, which won't be in this list.
        auto ownBuffers = tree->getSourceBufferIds();
        for (auto macro : tree->getDefinedMacros()) {
            ...
            byName[name] = Contribution{tree, macro};
        }
    }

    std::vector<const syntax::DefineDirectiveSyntax*> newPool;
    std::vector<std::shared_ptr<syntax::SyntaxTree>> newTrees;
    flat_hash_set<syntax::SyntaxTree*> seenTrees;
    newPool.reserve(byName.size());

    // Order-independent content hash: combine each (name, raw source text) pair into its own
    // sub-hash, then aggregate sub-hashes with `+=` so the total doesn't depend on byName's
    // (unordered) iteration order. Two rebuilds with the same macro set must hash equal even if
    // the hash_map iterates them in a different order.
    size_t hash = 0;
    for (auto& [name, contribution] : byName) {
        newPool.push_back(contribution.macro);
        if (seenTrees.insert(contribution.tree.get()).second)
            newTrees.push_back(contribution.tree);

        size_t h = std::hash<std::string_view>{}(name);
        h ^= std::hash<std::string>{}(contribution.macro->toString()) + 0x9e3779b9 + (h << 6) +
             (h >> 2);
        hash += h;
    }
    ...
    m_macroPoolTrees = std::move(newTrees);
    ++m_macroPoolGen;
}

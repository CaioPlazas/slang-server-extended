...
void ServerDriver::analyzeDocument(SlangDoc& doc, const lsp::RequestContext& ctx) {
    ...
    //
    // Upstream v0.3.0 replaced updateDoc(doc, FileUpdateType) with this function and dropped the
    // full-compilation-on-save branch entirely, so the refresh is now just a re-analysis.
    if (doc.isIncludeFragment()) {
        for (auto& owner : doc.getIncludeOwners()) {
            if (isDocumentOpen(owner->getURI())) {
                analyzeDocument(*owner, ctx);
            }
        }
    }
}
...
StandaloneParse classifyStandaloneParse(const std::shared_ptr<SlangDoc>& doc) {
    ...
    // (b) Declares only classes -- e.g. a UVM component/object header meant to be `\`include`d
    // into a package. Treated as a standalone unit: binding it to its owner's context was the
    // `experimental.uvmVerificationLinting` opt-in, withdrawn in v0.5.5.
...
void ServerDriver::openDocument(const URI& uri, const std::string_view text) {
    ...
    // Distinct from matchesBuildSource: that asks "is this file part of the compilation" (and
    // gates shallow diags); this asks "was this an already-known, unchanged build member" (and
    // gates the orphan-fragment scan at the end).
    ...
    if (docIter != docs.end() && docIter->second->textMatches(text)) {
        ...
        if (!doc->isIncludeFragment()) {
            if (classifyStandaloneParse(doc) == StandaloneParse::Fragment) {
        ...
        // m_buildSourceUris is populated from every BUFFER of every tree, so it also contains
        // files that are only ever `\`include`d -- a `.vh`/`.svh` fragment is a buffer of its
        // owner's tree just as much as the owner's own file is. Caching such a file here as a
        // standalone compilation unit is what this fork's resolveIncludeFragments exists to
        // avoid, so let genuine fragments fall through to resolveDocument below, which binds
        // them to their owner(s) instead. Files that parse as real units keep upstream's fast
        // path: they are already in the compilation, so their diagnostics are published from it.
        if (classifyStandaloneParse(doc) == StandaloneParse::Fragment) {
            doc.reset();
        }
        else if (doc->textMatches(text)) {
...
void ServerDriver::adoptOrphanFragments(const std::shared_ptr<SlangDoc>& newOwner) {
    ...
    // Collect matches first, then mutate/re-analyze afterwards -- analyzeDocument can transitively call
    ...
    for (auto& [uri, doc] : docs) {
        ...
        if (!doc->isIncludeFragment() &&
            classifyStandaloneParse(doc) == StandaloneParse::Standalone) {
            continue;
        }

...
void Indexer::extractFromRoot(const slang::syntax::CompilationUnitSyntax& root,
    ...
    // Classes aren't ModuleDeclarationSyntax members of the compilation unit, so they need to be
    // collected separately -- UVM-style testbench files frequently contain nothing else, and
    // without this they'd contribute no symbols at all.
    for (auto* classDecl : meta.classDecls) {
        ...
        if (!name.empty()) {
            dest.symbols.push_back(
                GlobalSymbol{.name = std::string(name), .kind = classDecl->kind});
        }
    }
...
std::vector<Indexer::IndexedPath> Indexer::indexPaths(const std::vector<fs::path>& paths) const {
    ...
    auto processRange = [&loadResults, &paths](size_t start, size_t end) {
        ...
        for (size_t i = start; i < end; i++) {
            ...
            // A file with no design unit is only ever `\`include`d into another file, and its
            // references are already attributed there via the includer's expansion. Recording
            // them here as well would report every such reference twice.
            if (meta.nodeMeta.empty()) {
                dest.referencedSymbols.clear();
            }

            // Extract macros only if no global symbols were found (header files).
            if (dest.symbols.empty()) {
...
void Indexer::removeIndexEntries(const fs::path* uriPtr) {
...
void Indexer::indexPath(const fs::path& path, IndexedPath& indexedFile) {
    ...
    // A re-index (any .slang/*.json save triggers one) would otherwise append a second copy of
    // every entry for this file on top of the entries already there.
    removeIndexEntries(uriPtr);

...
- **Bundled Linux binary** — both published VSIXes, `linux-x64` and the untargeted universal
  one, ship the `slang-server` binary directly inside the extension, so installing it needs no
  separate server download or `PATH` setup.
...
elaborated hierarchy, select the active instance of each module, inspect instance-specific
values, and go back and forth with a waveform viewer.
...
A personal fork, used daily against real ASIC/FPGA workspaces and published to the
Marketplace. **v0.6.0 is the current stable release** — a plain install gets it, no
pre-release opt-in needed. Newer work lands on the pre-release channel first (tick *Switch to
Pre-Release Version* in the extension pane); v0.7.0 there carries the upstream v0.3.0 merge.
Honest about where the edges are:

- **The server is Linux x86-64 only.** Both published packages carry that one binary, so a
  Linux install works whichever package VS Code resolves. On macOS or Windows there is no
  server to bundle: supply your own via `slangCustom.path`, `PATH`, or the managed
  GitHub-release download. Up to and including v0.4.0 the universal package shipped with no
  binary at all, which left some Linux installs — notably behind Remote-SSH, where the gallery
  lookup runs on the Windows UI host — resolving that package and finding no server. Fixed for
  everyone in v0.6.0.
- **CI is red and has been for a while**, because the `external/slang` submodule points at
  a private repo the Actions runner cannot read. Everything is verified locally instead.
  It also means the weekly upstream sync has not been landing on its own.
- **The fork's additions are additive and off by default** unless noted, so upstream
  behaviour is what you get until you opt in. That is deliberate, and it is what keeps
  syncing from upstream cheap.
- Contributions of these ideas back to upstream are welcome — open an issue or reach out.
...
respective authors. Thanks to
the Hudson River Trading team and the Slang project for the foundation this builds on.

## License

MIT, inherited from upstream. See [LICENSE](LICENSE) and `REUSE.toml`.

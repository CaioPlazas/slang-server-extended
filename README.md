...
Marketplace on both channels: v0.4.0 is the current stable, and the `0.5.x` line ships as
**pre-release** (tick *Switch to Pre-Release Version* in the extension pane to get it).
...
- **The server is Linux x86-64 only.** Both published packages carry that one binary, so a
  ...
  GitHub-release download. Up to v0.5.3 the universal package shipped with no binary at all,
  which left some Linux installs — notably behind Remote-SSH, where the gallery lookup runs on
  the Windows UI host — resolving that package and finding no server. Fixed in v0.5.4.

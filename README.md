...
Marketplace as **pre-release versions only**. Honest about where the edges are:

- **The bundled server binary ships only in the `linux-x64` package.** On any other
  platform you get the universal package and supply a server yourself, via `slang.path`,
  `PATH`, or the managed GitHub-release download.

...
private:
    ...
    /// Superseded harvests, kept alive for the driver's lifetime rather than destroyed.
    /// A pooled macro expanded into a document's parse leaves tokens whose text points into the
    /// harvest buffer the macro was DEFINED in -- a string literal inside a macro body, say. Those
    /// documents reparse lazily, so releasing the old harvest as soon as its file is re-harvested
    /// frees text that live syntax trees still reference; it surfaces as a use-after-free during
    /// elaboration (StringLiteral::fromSyntax). Bounded by re-harvests per driver, and the driver
    /// is recreated whenever the config changes.
    std::vector<HarvestedFile> m_retiredHarvests;

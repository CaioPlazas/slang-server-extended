...
public:
    ...
    const std::vector<const syntax::DefineDirectiveSyntax*>& getMacroPool() const {
        return m_macroPool;
    }
...
private:
    ...
    /// Trees whose allocators own the DefineDirectiveSyntax pointers in m_macroPool below --
    /// pinned so the pool never dangles. Only trees that actually contributed a macro are pinned.
    std::vector<std::shared_ptr<syntax::SyntaxTree>> m_macroPoolTrees;
    std::vector<const syntax::DefineDirectiveSyntax*> m_macroPool;
    ...
    /// Re-entrancy guard: true while rebuildMacroPool() is walking m_buildOrder and calling
    /// getSyntaxTree() on each doc. While true, getMacroPool()/getMacroPoolGen() must return the
    /// previous (still-valid, still-pinned) pool instead of the one being built, so that parsing
    /// a pool contributor during the rebuild can't observe a half-built pool or recurse.
    bool m_rebuildingMacroPool = false;

    /// Recomputes m_macroPool from every doc in m_buildOrder. No-op unless the singleUnitMacros experimental toggle is on. See rebuildMacroPool() in ServerDriver.cpp for details.

...
private:
    ...
    // Drops every global-index entry previously recorded for this (interned) path. Must run
    // before re-adding entries for it, or repeated indexing accumulates duplicates.
    void removeIndexEntries(const std::filesystem::path* uriPtr);

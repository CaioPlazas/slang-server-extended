...
struct Config {
    ...
    rfl::Description<
        "Directory to treat as the effective working directory when resolving relative "
        "paths in `flags` (e.g. `-F ./my_flist.f`, `-I ../include`), mirroring "
        "`cd <workDir> && <tool> <flags>`. Relative to the workspace root; absolute paths "
        "are also accepted. Useful for ASIC flows where tools are conventionally run from "
        "a shared work directory rather than the repo root.",
        std::optional<std::string>>
        workDir;
    rfl::Description<
        "When a file that's only ever `include`d elsewhere (e.g. a `.vh`/`.svh` fragment "
        "meant to be spliced into a module body) is opened directly, analyze it in the "
        "context of the file(s) that include it instead of parsing it as a standalone "
        "top-level file. Disable to restore the previous behavior, where such files are "
        "always parsed on their own.",
        bool>
        resolveIncludeFragments = true;
    ...
    struct Experimental {
        rfl::Description<
            "Improve analysis of UVM-style verification code. (1) Binds class-declaring "
            ".svh files (e.g. UVM component/object headers) that are `include`d inside a "
            "package, module, or similar design unit to that owner's context instead of "
            "parsing them standalone. (2) Follows virtual-interface member types and "
            "class `extends` base-class names across files when resolving a shallow "
            "compilation's dependencies. Off by default; behavior may change or be "
            "removed without notice while this is validated.",
            bool>
            uvmVerificationLinting = false;
        rfl::Description<
            "Approximate `--single-unit` preprocessor behavior: predefine the union of all "
            "`define`s found in the build's files when parsing each file, so macros defined "
            "in one file (e.g. `uvm_macros.svh` pulled in by a package) resolve in files "
            "that don't `include` them. Symbol scoping is unchanged. Order-insensitive, so "
            "a macro defined in a later file is visible in an earlier one. Off by default.",
            bool>
            singleUnitMacros = false;
    };

    rfl::Description<"Experimental / beta features. Off by default; interfaces here may "

...
private:
    ...
    /// Generation of the driver's macro pool (see ServerDriver::getMacroPoolGen, the
    /// `singleUnitMacros` experimental feature) this tree was last parsed against. getSyntaxTree()
    /// reparses when this no longer matches the driver's current value.
    uint64_t m_macroGen = 0;
    ...
    /// Owners whose `` `define``s should be predefined when this document parses its own,
    /// independent syntax tree -- unlike m_includeOwners, this document still gets its own
    /// tree (it's a real compilation unit, or a fragment candidate that couldn't be safely
    /// spliced into any owner's tree for symbol resolution, e.g. a class-only `.svh`
    /// `` `include``d at $unit scope rather than nested in a design unit). Macro state doesn't
    /// carry the same symbol-scoping risk that a full splice does, so it can be inherited more
    /// permissively. See ServerDriver::resolveDocument/openDocument/adoptOrphanFragments.
    std::vector<std::shared_ptr<SlangDoc>> m_macroOwners;
...
public:
    ...
    /// Register `owner` as a source of `` `define``s to predefine when this document parses its
    /// own syntax tree (see m_macroOwners). No-op if `owner` is already known. Invalidates the
    /// cached tree so the next getSyntaxTree() call reparses with the new macro predefined.
    void addMacroOwner(std::shared_ptr<SlangDoc> owner);
...
private:
    ...
    /// @brief Collects the `` `define``s of every doc in m_macroOwners, to predefine when
    /// parsing this document's own independent syntax tree. See m_macroOwners.
    std::vector<const slang::syntax::DefineDirectiveSyntax*> collectInheritedMacros() const;
};

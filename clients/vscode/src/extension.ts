...
File input is sent to stdin, and formatted output is read from stdout.',
  ...
  path: PathConfigObject = new PathConfigObject(
    ...
    },
    {
      ...
      installer: {
        githubRepo: 'CaioPlazas/slang-server-extended',
        assetNames: {
  ...
  async setupLanguageClient(): Promise<void> {
    ...
    this.client = new LanguageClient('slang-server-extended', serverOptions, clientOptions)
    const clientInfo = {
    ...
    // Note: the extension's own package.json version and the bundled/managed slang-server
    // binary's version are independent numbering schemes (the server's tracks this repo's C++
    // release history, not the VS Code extension's). There is deliberately no client/server
    // version-compatibility gate here -- a prior major.minor compatibility check caused a real
    // Marketplace outage (extension bumped past the bundled server's line, tripping a false
    // "server is older than minimum required" warning on every activation) and was removed
    // rather than re-synced, since the two version numbers aren't meant to move in lockstep.
    //
    // UPSTREAM REINTRODUCES THIS ON EVERY MERGE -- last stripped when merging upstream v0.3.0.
    // Upstream keeps its client version BELOW its server version (theirs: client 0.2.17, server
    // 0.3.0) so their check never fires. Ours is permanently above: the extension version is a
    // Marketplace version and cannot move backwards, while the server's tracks upstream's C++
    // releases. Re-adding upstream's block would pop "server is older than minimum required" for
    // every user on every activation. Keep it out.
...
export async function activate(context: vscode.ExtensionContext) {
  ...
  await ext.activateExtension('slangCustom', 'Slang Custom', context, [

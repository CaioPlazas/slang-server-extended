  ...
  <form id="form">
    <section>
      ...
      <div class="field">
        <span>Flags</span>
        <small class="muted">
          Flags to pass to slang, e.g. <code>-f</code>/<code>-F project.flist</code>. Use
          <code>-I</code> to add header/include search directories. Append
          <code>...</code> after a directory to search it recursively, e.g.
          <code>-I rtl/...</code>.
        </small>
    ...
    <section>
      ...
      <div id="uvm-fields">
        <div class="field">
          ...
          <small class="muted">
            Folders to search for UVM includes, applied as <code>-I</code> flags. Check
            "Recursive" to search a folder's subdirectories too.
          </small>
...
<template id="flag-row-template">
  <div class="row flag-row">
    ...
    <input class="flag" type="text" placeholder="e.g. -I rtl/..." />
  </div>
</template>
...
<template id="uvm-dir-row-template">
  <div class="row uvm-dir-row">
    <button type="button" class="remove" title="Remove">&times;</button>
    <div class="input-row">
      <input class="path" type="text" placeholder="e.g. uvm/src" />
      <button type="button" class="secondary browse">Browse…</button>
    </div>
    <label class="field inline">
      <span>Recursive</span>
      <input class="recursive" type="checkbox" />
    </label>
  </div>
</template>
...
(function () {
  ...
  function addFlagRow(value) {
    const node = addRow('flag-row-template', 'flags-rows');
    node.querySelector('.flag').value = value || '';
  }
  ...
  function addUvmDirRow(path, recursive) {
    ...
    node.querySelector('.recursive').checked = !!recursive;
  ...
  // Pulls "-I <dir>" pairs (recursive if <dir> ends in "/...") and a bare uvm.sv/uvm_pkg.sv-ish
  // token out of a flags string, for the friendlier UVM-specific rows above. Whatever's left
  // over is handed to the generic Flags row list, so the same "-I" doesn't show up twice.
  ...
  function extractUvmFlags(flagsStr) {
    ...
    for (let i = 0; i < tokens.length; i++) {
      ...
      if (tok === '-I' && i + 1 < tokens.length) {
        ...
        dirs.push({ path: p, recursive: recursive });
        i++;
        continue;
      }
    ...
    return { dirs: dirs, uvmFile: uvmFile, rest: rest };
  }
  ...
  function loadConfig(config) {
    ...
    const uvmParsed = extractUvmFlags(config.flags);
    $('uvm-dir-rows').innerHTML = '';
    uvmDirRowsById.clear();
    uvmParsed.dirs.forEach((d) => addUvmDirRow(d.path, d.recursive));
    $('uvmFile').value = uvmParsed.uvmFile || '';

    $('flags-rows').innerHTML = '';
    uvmParsed.rest.forEach(addFlagRow);
  ...
  function readConfig() {
    ...
    const flagValues = [];
    document.querySelectorAll('#flags-rows .flag-row .flag').forEach((input) => {
      const v = input.value.trim();
      if (v.length > 0) flagValues.push(v);
    });
    ...
    // generated section, not data loss.
    document.querySelectorAll('#uvm-dir-rows .uvm-dir-row').forEach((row) => {
      const p = row.querySelector('.path').value.trim();
      if (p.length === 0) return;
      const recursive = row.querySelector('.recursive').checked;
      flagValues.push('-I', recursive ? p + '/...' : p);
    });
    ...
    config.experimental = { uvmVerificationLinting: $('uvmVerificationLinting').checked };

...
export class PathConfigObject extends ConfigObject<string> {
   ...
   * 2. Explicit `slang.path` user setting (the override for a custom/non-bundled binary)
   * 3. Binary bundled directly in this extension's install (the default, shipped experience)
   * 4. Cached value / PATH
   * 5. Previously installed binary in extension storage
   * 6. GitHub release installer (if configured)
   */
  async resolveToolPath(
    ...
    // 2. Check explicit `slang.path` setting -- the escape hatch for a custom/non-bundled binary
    const explicitPath = vscode.workspace.getConfiguration().get(this.configPath!, '')
    if (explicitPath !== '') {
      const found = await this.findToolPath()
      log.info(`Using ${toolName} at ${found}`)
      return found
    }

    // 3. Check for a binary bundled directly in this extension's install
    const bundledPath = path.join(context.extensionPath, 'bin', toolName)
    if (await this.isExecutableFile(bundledPath)) {
      log.info(`Using bundled ${toolName} at ${bundledPath}`)
      this.cachedValue = bundledPath
      return bundledPath
    }

    // 4. Check cached value / PATH
    ...
    // 5. Check if already installed in extension storage
    if (config) {

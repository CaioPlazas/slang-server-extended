...
Newest first. `release.sh` reads the section matching the version being released and uses
its heading as the GitHub Release title and its body as the release notes, so a release
with no section here fails rather than shipping blank notes.

All major and minor changes to this project are documented in this file.
...
## v0.7.0 (pre-release)
Merges **upstream slang-server v0.3.0** — 50 commits of server work since the v0.2.10 that
v0.6.0 shipped. This is a large change, so it goes out on the pre-release channel first;
v0.6.0 remains the stable release and a default install still gets it.

* **Completions were rewritten upstream.** Adjacent tokens now drive which kind of completion
  you get, macros are filtered, global typedefs and classes are offered, nested struct members
  are shown, multidimensional instance arrays are handled, and interface port arrays resolve
  even when their bounds are not evaluable.
* **Hierarchy work moved server-side**, with a chained hierarchy request, hierarchy search,
  interface ports exposed in the view, and "active instance" selection — pick which instance of
  a module you are looking at, and hovers, inlay hints and gotos follow that choice.
* **Cone tracing** for drivers and loads through the call hierarchy.
* **Struct assignment patterns**: better diagnostics for nested struct assignment problems,
  inferred assignment-pattern type hints, and support for incomplete struct types.
* **Hovers**: file links for the types of values, declared types for incomplete symbols, macro
  usages shown for macro-generated drivers, resolved interface type parameters, and macro-hover
  fixes for defines that came from the command line.
* **The server can now cancel in-flight work.** Long requests are queued and superseded edits
  are abandoned instead of blocking later ones, so a burst of typing no longer stacks up.
* Two new inlay-hint options, `inlayHints.assignmentPatternTypes` and
  `inlayHints.activeParameterValues`, both on by default.
* **This fork's own features are unchanged**: `resolveIncludeFragments` (on by default),
  `workDir`, the CentOS 7 / glibc 2.17 static binary bundled in both packages, the separate
  extension identity, and the server configuration panel. `experimental.uvmVerificationLinting`
  stays removed, as in v0.6.0.
* **Fixed: the welcome view's buttons did nothing.** "Select Build File", "Select Top Level" and
  "Configure Slang" referenced command ids that this fork never registers under that name, so
  clicking them was silently a no-op. Pre-existing, found while merging.

## v0.6.0
First stable release since v0.4.0. Everything below has been on the pre-release channel for
some weeks and is in daily use; this is the same content, promoted so that a default install
gets it. If you were already on pre-release, nothing here is new to you.

* **The extension now bundles the server binary in *both* published packages, which fixes
  "slang-server not found" on a default install.** VS Code falls back to the untargeted
  "universal" package whenever it cannot resolve the platform-specific one -- which is exactly
  what happens on a Linux remote when the gallery lookup runs on a Windows UI host, as with
  Remote-SSH or WSL. Up to and including v0.4.0 that package shipped with no server in it, so
  those installs came up with nothing and offered to download a server. Both packages now carry
  it. Linux x64 users who resolved the platform-specific package were never affected.
* **The shipped binary is now built from the commit it ships in.** Some earlier releases carried
  a server compiled at an older commit, so server-side fixes announced in their notes were not
  actually in the package. The release script now refuses to publish a binary whose `--version`
  does not match the released commit. You can check any published package yourself: unzip it and
  run `extension/bin/slang-server --version`.
* **Merged upstream [slang-server v0.2.10](https://github.com/hudson-trading/slang-server/releases).**
  Better hover headers for definitions, instances and drivers; port direction shown in hover
  headers; macro-hover typedef resolution; interface port completions; correct source locations
  for diagnostic notes originating behind macros; and two slang bumps.
* **`experimental.uvmVerificationLinting` is removed.** This one is a genuine removal if you are
  coming from v0.4.0, where the option existed. It bound class-only `.svh` files to the package
  that `` `include``d them and followed `virtual interface` member types and class `extends` base
  names across files. In real testbench use it misbehaved often enough not to be worth keeping,
  and it never handled the harder cases anyway -- types declared in a file unreachable through
  the owner's own scope, and macro inheritance between files sharing no `` `include``
  relationship. Class-only `.svh` files are parsed standalone again, which is the upstream
  behaviour. If you have the key in a `.slang/server.json` you do not need to do anything: it is
  ignored. Remove it to silence the editor's schema warning.
* **`resolveIncludeFragments` and `workDir` are unchanged**, and `resolveIncludeFragments`
  remains on by default. Genuinely empty or `` `include``-only fragments are still bound to
  their owners; that is a separate mechanism from the option removed above.

## v0.5.5 (pre-release)
* **The `experimental.uvmVerificationLinting` option is removed.** It bound class-only `.svh`
  files to the package that `` `include``d them, and followed `virtual interface` member types
  and class `extends` base names across files. In real testbench use it misbehaved often enough
  not to be worth keeping, and it never handled the cases that matter most anyway -- types in a
  file unreachable through the owner's own scope, and macro inheritance between files that share
  no `` `include`` relationship. If you set it in a `.slang/server.json`, it is now an unknown
  key and is ignored; remove it to silence the schema warning. The VS Code config panel's
  "(Experimental) UVM & Verification Linting" checkbox is gone with it, though the UVM include
  folders and `uvm.sv` fields remain -- those were only ever a friendlier view over `flags` and
  work the same as before.
* **`resolveIncludeFragments` is unaffected and stays on by default.** Genuinely empty or
  `` `include``-only fragments are still bound to their owners; that is a separate mechanism and
  none of it changed. Class-only `.svh` files simply go back to being parsed standalone, which is
  the v0.4.0 and upstream behaviour.
* No other change. The server is otherwise identical to v0.5.4.

## v0.5.4 (pre-release)
* **The extension now bundles the server binary in *both* published packages.** The untargeted
  "universal" package shipped without one, on the reasoning that a Linux ELF is dead weight on
  Windows and macOS. In practice VS Code falls back to that package whenever it cannot resolve
  the platform-specific one -- which is what happens on a Linux remote when the gallery lookup
  runs on a Windows UI host -- so those installs came up with no server at all and prompted to
  download one. If you saw *"slang-server not found"*, this is why. There is no macOS or Windows
  server build to fall back to, so nothing is lost by carrying the binary everywhere.
* **The bundled binary is now built from the release it ships in.** Versions 0.5.1 through 0.5.3
  each shipped a server compiled at the v0.3.3 commit, so none of the server-side work announced
  in those releases was ever actually in them -- including v0.5.3's own crash fix. The release
  script now refuses to package a binary whose `--version` does not match the commit being
  released. You can check any published package yourself: unzip it and run
  `extension/bin/slang-server --version`.
* **The `experimental.singleUnitMacros` option and the 0.5.x UVM/verification work are
  withdrawn.** The macro-pool machinery did not behave acceptably, and because of the stale-binary
  bug above it had never actually run in a released package. The server is back to the v0.4.0
  baseline. If you had `experimental.singleUnitMacros` set in a `.slang/server.json`, it is now
  an unknown key and is ignored; remove it to silence the schema warning. `experimental.uvmVerificationLinting`
  and the include-fragment resolution features are unaffected -- they were already in v0.4.0.
* Merged upstream [slang-server v0.2.10](https://github.com/hudson-trading/slang-server/releases):
  better hover headers for definitions, instances and drivers, port direction shown in hover
  headers, macro-hover typedef resolution, interface port completions, correct source locations
  for diagnostic notes originating behind macros, and two slang bumps.
* No behaviour change for Linux x64 users on the platform-specific package: VS Code always prefers
  it over the universal fallback.

## v0.5.3 (pre-release)
* **Fixes a crash introduced in v0.5.2.** With `experimental.singleUnitMacros` enabled, saving a
  file crashed the server roughly half the time. Two lifetime bugs, both found with
  AddressSanitizer:
    * The pool rebuild returned early when the macro content was unchanged — the common case for a
      save — leaving freed `DefineDirectiveSyntax` pointers in the pool after the re-harvest had
      already destroyed the tree that owned them.
    * Destroying a superseded harvest freed source buffers that live document trees still pointed
      into. A pooled macro expanded into a document leaves tokens whose text lives in the buffer
      the macro was *defined* in (a string literal inside a macro body, say), and documents reparse
      lazily, so that text has to outlive the harvest. Superseded harvests are now retired rather
      than freed.
  Verified with 8/8 clean save cycles and 3/3 clean AddressSanitizer runs.
* Measured `singleUnitMacros` cost on `examples/uvm_apb_sram` (199 files, release build), on vs
  off: startup unchanged, first file open about 65 ms slower, saves unchanged. Documented in the
  [config reference](docs/start/config.md).

## v0.5.2 (pre-release)
* `experimental.singleUnitMacros`: macros from **include-guarded** headers (the shape real UVM
  ships, e.g. `uvm_macros.svh`) now resolve reliably. The pool was previously harvested from the
  documents' own syntax trees, which were themselves parsed *with* the pool applied — so once a
  header's guard macro was pooled, that header's `` `ifndef`` went false, its body was skipped, and
  its macros dropped out of the pool, reappearing on the next save. Macros are now harvested from
  a separate per-file parse that inherits nothing, so guards always evaluate normally.
* `experimental.singleUnitMacros`: a save no longer re-harvests the whole build and reparses every
  file. Only the saved file and any build file that `` `include``s it are re-harvested, and the pool's
  content hash still gates whether anything downstream reparses at all.
* `experimental.singleUnitMacros`: a file's own macros are no longer predefined back into its own
  parse, so a self-guarded file cannot suppress its own body.
* `experimental.singleUnitMacros`: warns when enabled with no build/flist configured, where the
  pool has nothing to harvest and the option silently does nothing.
* Fixed stale analysis after a macro-pool change: the document's syntax tree was replaced while its
  analysis was left built against the superseded buffer, which could silently drop diagnostics.
* Added `ScopedTimer` instrumentation to build parsing, macro-pool rebuilds, and compilation
  refresh.

## v0.5.1 (pre-release)
* Indexer: files containing only class declarations -- the standard UVM testbench shape under
  `tb/`, `agents/`, `env/`, `sequence_lib/` -- were parsed and then discarded entirely,
  contributing no symbols, macros, or references. They are now indexed, and class names are
  recorded as symbols for the first time. This fixes goto-definition, find-references, workspace
  symbols, and completions for verification code.
* Indexer: files created or deleted after startup are now picked up; the watcher previously
  subscribed to change events only, so new files needed a server restart to become navigable.
* Indexer: index entries no longer duplicate on every re-index (triggered by any `.slang/*.json`
  save).
* `experimental.singleUnitMacros`: documented its performance cost and known defects. See
  [config reference](docs/start/config.md); it remains off by default.
* Build: pnpm 10 stopped reading the `pnpm` field in `package.json`, silently dropping the
  dependency security overrides on any fresh install. Moved them to `pnpm-workspace.yaml` and
  pinned `packageManager`.

## v0.5.0 (pre-release)
* Re-publish of v0.3.3's content under a new version number, no code changes.
* Marketplace listing fix: v0.3.3 was published with the pre-release flag while sitting above
  v0.3.2 (stable, no flag) in version order. This left no valid version for users on the default
  Stable update channel, and VS Code showed "This extension is no longer available on the
  Extension Marketplace" for everyone, regardless of platform. v0.5.0 re-publishes the same
  content as a pre-release version above the new stable baseline (v0.4.0, a re-publish of
  v0.3.2's content) so the pre-release channel stays ahead of stable, avoiding the same trap.

## v0.4.0
* Re-publish of v0.3.2's content under a new version number, no code changes.
* Marketplace listing fix: see v0.5.0 above for the full explanation. v0.4.0 re-establishes an
  even-minor stable version above the whole `0.3.x` line to fix the listing.

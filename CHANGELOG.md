...
* Marketplace listing fix: the `0.3.x` line used an odd minor version (the documented
  pre-release convention), and v0.3.3 was published with the pre-release flag while sitting
  above v0.3.2 (stable, no flag) in version order. This left no valid version for users on the
  default Stable update channel, and VS Code showed "This extension is no longer available on
  the Extension Marketplace" for everyone, regardless of platform. v0.4.0 re-establishes an
  even-minor stable version above the whole `0.3.x` line to fix the listing. v0.3.3's content was
  separately re-published as v0.5.0 (pre-release) on `main`, so the pre-release channel stays
  above this stable baseline.

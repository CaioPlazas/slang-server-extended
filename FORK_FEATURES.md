...
Both published VSIXes ship the `slang-server` binary directly inside the extension package: the
platform-specific `linux-x64` one and the untargeted universal one. Install either and it just
works — no download prompt, no separate `slang-server` install on your system.

Both carry it because VS Code does not always resolve the platform-specific package. Behind
Remote-SSH the gallery lookup runs on the *client* machine, so a Windows client driving a Linux
remote resolves the universal package — which, up to and including v0.4.0, contained no server
at all and prompted you to download one. The binary is Linux x86-64 either way; on macOS and Windows it is
unused ballast, which is the cheaper of the two mistakes.

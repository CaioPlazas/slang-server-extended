...
TEST_CASE("SingleUnitMacrosGuardedHeaderSurvivesRepeatedSaves") {
    // Regression: the pool used to be harvested from the docs' own trees, which were themselves
    // parsed WITH the pool. Once GUARDED_MACROS_SVH was pooled, guarded_macros.svh's `\`ifndef`
    // went false, its body (and its `\`define`s) was skipped, and GUARDED_UTILS dropped out of the
    // pool -- reappearing on the next save, oscillating forever. Every other fixture in this
    // directory is guard-free, which is exactly why the bug shipped.
    ServerHarness server("single_unit_macros");
    server.loadConfig(
        Config{.build = "build.f", .experimental = Config::Experimental{.singleUnitMacros = true}});

    auto user = server.openFile("guarded_user.sv");
    CHECK_FALSE(hasDiagMentioning(user.getDiagnostics(), "GUARDED_UTILS"));

    // The old bug flipped on alternate saves, so one save wouldn't have caught it.
    auto holder = server.openFile("guarded_holder_pkg.sv");
    for (int i = 0; i < 3; i++) {
        holder.save();
        CHECK_FALSE(hasDiagMentioning(user.getDiagnostics(), "GUARDED_UTILS"));
    }
...
TEST_CASE("SingleUnitMacrosOffGuardedUserReportsUnknownMacro") {
    // Negative control for the guarded-header test above: with the toggle off, guarded_user.sv
    // has no way to see GUARDED_UTILS. Without this, that test could pass simply because the
    // macro was resolving through some path other than the pool.
    ServerHarness server("single_unit_macros");
    server.setBuildFile("build.f");

    auto user = server.openFile("guarded_user.sv");
    CHECK(hasDiagMentioning(user.getDiagnostics(), "GUARDED_UTILS"));
}

TEST_CASE("SingleUnitMacrosSelfGuardedFileKeepsItsBody") {
    // A file defining its own include guard contributes that guard to the pool; collectInheritedMacros
    // excludes a file's own contributions so the guard can't be predefined back into its own parse
    // and collapse the body.
    //
    // NOTE: this is a regression guard, not a proof of that filter -- it still passes with the
    // filter disabled, so the collapse it describes evidently needs some further condition to
    // reproduce. Kept because a future change that does collapse self-guarded files would fail it.
    ServerHarness server("single_unit_macros");
    server.loadConfig(
        Config{.build = "build.f", .experimental = Config::Experimental{.singleUnitMacros = true}});

    auto pkg = server.openFile("self_guarded_pkg.sv");
    // If the guard suppressed the body, the file parses to nothing and has no symbol tree.
    CHECK_FALSE(pkg.getSymbolTree().empty());
}

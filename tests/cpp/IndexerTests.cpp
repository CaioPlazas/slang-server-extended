...
TEST_CASE("Index class declarations (class-only files)") {
    // Files containing only classes and no design unit are the standard UVM testbench shape.
    // These used to be parsed and then discarded entirely, contributing nothing to the index.
    Indexer indexer;
    auto testPath = getTestDataPath();
    indexer.startIndexing({testPath + "/classes.sv"}, {});

    auto files = indexer.getFilesForSymbol("MyClass");
    CHECK(files.size() == 1);

    files = indexer.getFilesForSymbol("AnotherClass");
    CHECK(files.size() == 1);
}

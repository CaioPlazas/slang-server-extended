...
The Linux build bundles a CentOS 7 / glibc 2.17-compatible server binary, so no separate server install is needed on most systems. You can also grab the VSIX directly from this fork's [Releases page](https://github.com/CaioPlazas/slang-server/releases) and install it with `code --install-extension slang-server-extended-linux-x64.vsix`. Alternatively, build slang-server yourself and set `slangCustom.path` to that binary. See [FORK_FEATURES.md](https://github.com/CaioPlazas/slang-server/blob/main/FORK_FEATURES.md) for this fork's additions.

### Vscode Forks (Cursor, Antigravity, VSCodium, etc.)

Install the same VSIX from this fork's [Releases page](https://github.com/CaioPlazas/slang-server/releases) using your editor's "Install from VSIX" command.

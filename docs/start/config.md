...
:   **Type:** `boolean`
    ...
    **Default:** `true`
    ...
    When a file that's only ever `` `include``d elsewhere (e.g. a `.vh`/`.svh` fragment meant to be spliced into a module body, not a standalone compilation unit) is opened directly, analyze it in the context of the file(s) that include it instead of parsing it on its own. This avoids spurious "expected a module" style errors on such files, and gives real diagnostics based on the including module(s)' declarations. If the same fragment is included by more than one file, diagnostics are the union of up to 8 including contexts (capped for performance; features other than diagnostics, like goto-definition and hover, use only the first-found owner).
...
:   **Type:** `Experimental`
    ...
    interface Experimental {
      /** Extend resolveIncludeFragments to also bind class-declaring .svh files included
       *  inside a package/module/etc. Off by default. */
      uvmVerificationLinting?: boolean  // default: false
      /** Approximate --single-unit preprocessor behavior: predefine the union of all
       *  `define`s found in the build's files when parsing each file. Off by default. */
      singleUnitMacros?: boolean  // default: false
    }
    ```

    Beta features that are off by default and may change or be removed without notice.

    - **`uvmVerificationLinting`**: Extends `resolveIncludeFragments` to also bind class-declaring `.svh` files (e.g. UVM component/object headers) that are `` `include``d inside a package, module, or similar design unit, analyzing them in that owner's context instead of parsing them standalone. This targets the common UVM pattern of a class body living in its own `.svh`, `` `include``d into a package. See [upstream issue #135](https://github.com/hudson-trading/slang-server/issues/135) for background. Does **not** by itself fix cross-file macro inheritance without `--single-unit`, or resolution of types declared in a separate file that aren't reachable through the owner's own scope.

    - **`singleUnitMacros`**: Approximates slang's `--single-unit` preprocessor behavior: predefines the union of every `` `define`` found anywhere in the build's files when parsing each file, so macros defined in one file (e.g. `uvm_macros.svh` pulled in by a package) resolve in files that don't `` `include`` them directly -- for example a top-level `tb_top.sv` that relies on a sibling flist file to have already pulled in UVM macros, the way a real `--single-unit` compile would see them. Symbol scoping is unchanged; this only affects macro state. Order-insensitive, so a macro defined in a later file in the build is visible in an earlier one, and conflicting redefinitions resolve last-wins in flist order. A file's own `` `define`` always wins locally over the pool.

        !!! note "Performance"

            Measured on `examples/uvm_apb_sram` (199 files) with a release build, on vs off:
            server startup is unchanged, the first open of a file costs about 65 ms more, and
            saves are unchanged. Only the saved file and any build file that `` `include``s it are
            re-harvested, and the pool's content hash suppresses the downstream reparse when the
            macro set hasn't actually changed.

            Costs scale with the size of the build, so a much larger testbench will see more than
            the numbers above. To turn it off:
            `#!json { "experimental": { "singleUnitMacros": false } }`

        !!! warning "Limitation"

            The pool is harvested from the files the build actually parses, whether they come from
            a `build`/flist or from positional sources in `flags`. If nothing is parsed, the pool
            is empty and the option has no effect; the server logs a warning when that happens.

    **Example:**

    ```json
    { "experimental": { "uvmVerificationLinting": true } }
    ```

    ```json
    { "experimental": { "singleUnitMacros": true } }
    ```

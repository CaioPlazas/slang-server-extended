...
:   **Type:** `Experimental`
    ...
    - **`singleUnitMacros`**: Approximates slang's `--single-unit` preprocessor behavior: predefines the union of every `` `define`` found anywhere in the build's files when parsing each file, so macros defined in one file (e.g. `uvm_macros.svh` pulled in by a package) resolve in files that don't `` `include`` them directly -- for example a top-level `tb_top.sv` that relies on a sibling flist file to have already pulled in UVM macros, the way a real `--single-unit` compile would see them. Symbol scoping is unchanged; this only affects macro state. Order-insensitive, so a macro defined in a later file in the build is visible in an earlier one, and conflicting redefinitions resolve last-wins in flist order. A file's own `` `define`` always wins locally over the pool.
        ...
        !!! warning "Known issues (as of v0.5.0)"

            This option currently has significant costs and defects:

            - **Performance.** The macro pool is rebuilt on *every* save, and any change to it
              reparses every file in the build -- twice over in build mode. Expect noticeably
              slower saves and startup on large testbenches.
            - **Include-guarded headers.** Macros from a guarded header (the usual shape of
              `uvm_macros.svh`) can drop out of the pool and be reported as unknown, then come
              back on the next save.
            - **Explore mode.** With no `build`/flist configured the pool is always empty, so the
              option has no effect at all.

            If you hit any of these, turn it off:

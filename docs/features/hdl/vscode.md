...
The appearance is controlled by `slangCustom.inactiveRegions.style`:

- `"opacity"` (default) — Renders inactive regions at reduced opacity. Controlled by `slangCustom.inactiveRegions.opacity` (default 0.55).
- `"background"` — Highlights inactive regions with a background color. Controlled by `slangCustom.inactiveRegions.backgroundColor` (default `#1212124C`).
